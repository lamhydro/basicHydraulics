...
\begin{equation}
\color{red}\boxed{\color{black} Q_{ij} = \sum_{j=1}^{NT_i} \left(\frac{H_j - H_i }{\sum K_{m_{ij}}  + f_{ij}\frac{L_{ij}}{D_{ij}} } \right)^{\frac{1}{2}} \sqrt{2g} A_{ij} - Q_{Di} }
\label{cpne7}
\end{equation}
...
\begin{enumerate}
\item Definici\'on de la topolog\'ia de la red. Los circuitos estan conformados por tuber\'ias y la conecci\'on de las tuber\'ias en los circuitos es positiva siguiendo las manecillas del reloj y negativo en sentido contrario. Todos los tubos deben quedar incluidos en almenos un circuito. Definir los nodos con cabeza conocida. 
\item Si existe mas de un nodo con cabeza conocida es necesario conectarlos a trav\'es de una tuber\'ia imaginaria formando un circuito que los una. En estas tuber\'ias imaginarias se deben suponer sus caracter\'isticas con el fin de calcular el caudal hipot\'etico que circular\'ia por ellas. Sin embargo en las correciones de caudal, los tubos imaginarios no deben ser incluidos.
...
\item Se calcula la sumatoria de las p\'erdidas a lo largo del circuito, teniendo en cuenta que los flujos en sentido de las manecillas del reloj arrojan perdidas positivas y las flujos en sentido contrario dan p\'erdidas negativas. Si la sumatoria es diferente de cero, se procede a calcular la correcci\'on de caudal para las tuber\'ias del circuito con la ecuaci\'on~\ref{cpne15}. Note que el balance de perdidas se deben restar las cabezas de bombas que existiesen en alguna tuber\'ia del circuito. 
\item Los pasos 4 y 5 se repiten para el numerto de circuitos. Note que hay tubos comunes en circuitos lo que implica que el caudal por ellos debe estar corregido para el siguiente circuito en la iteraci\'on. 
...
Este m\'etodo se utiliza para resolver las ecuaciones de cabeza (ecuaci\'on~\ref{cpne7}) para los nodos de la red. Las ecuaciones del m\'etodo son las ecuaciones~\ref{cpne5}. Por lo tanto en lugar de suponer caudales en las tuber\'ias, se supone la cabeza en cada uno de los ($Nu-1$) nodos de la red. Note que la cabeza en almenos uno de los nodos debe ser conocida. Luego, se ajustan las cabezas en cada uno de los nodos y el proceso de ajuste se repite hasta que la ecuaci\'on de continuidad en los nodos (ecuaci\'on~\ref{cpne2}) se cumpla. 

Si se considera que la cabeza $H_i$ en un nodo $i$ esta sobrestimado o subestimada, el factor de ajuste de cabeza en un nodo $i$ es $\Delta H_i$, la ecuaci\'on~\ref{cpne5} se convierte en:
...
Analizando el termino $\left( (H_j - H_i)- \Delta H_i \right)^{1/2} $ utilizando el teorema del binomio, la ecuaci\'on~\ref{cpne16} se expresa como:
...
\begin{equation}
\Delta H_i = \frac{2(\sum Q_{ij}-Q_{Di})}{\sum \frac{Q_{ij}}{H_j - H_i }}
\label{cpne19}
\end{equation}
...
Teniendo en cuenta que la ecuaci\'on~\ref{cpne22} tiene la forma de una polin\'omio de grado 2, $aQ_{ij}^2 + bQ_{ij} + c$, $Q_{ij} = 0$, la soluci\'on es encontrar las ra\'ices reales del polinomio.
